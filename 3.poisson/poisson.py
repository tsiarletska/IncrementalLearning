...
while len(stream_values) < TOTAL_ITERATIONS:
    ...
    segment_vals = random_state.poisson(lam, Ti)
    
    # DYNAMIC RULE: Class 1 if the count is strictly greater than the current average
    segment_lbls = (segment_vals > lam).astype(int)
...
def plot_stream_and_hist(stream, drifts, segment_lengths, segment_colors, random_state):
    ...
    dist_info = [
        (2.0, "tab:blue", "Lam=2"),
        (5.0, "tab:orange", "Lam=5"),
        (10.0, "tab:green", "Lam=10"),
        (15.0, "tab:red", "Lam=15"),
        (20.0, "tab:purple", "Lam=20")
    ]
    
    for lam, color, label in dist_info:
        ideal_dist = random_state.poisson(lam, 2000)
        ax2.hist(ideal_dist, bins=15, alpha=0.5, label=label, color=color)
...
plt.title("Classification Accuracy Over 20,000 Iterations - Poisson Stream")
