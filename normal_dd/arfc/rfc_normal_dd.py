...
from river import naive_bayes, tree, linear_model, compose, preprocessing, forest
...
while len(stream_values) < TOTAL_ITERATIONS:
    ...
    if choice == 1:
        mu, sigma, color = 0.6258, 0.0470, "tab:blue"
    elif choice == 2:
        mu, sigma, color = 0.3745, 0.0204, "tab:orange"
    elif choice == 3:
        mu, sigma, color = 0.5760, 0.0288, "tab:green"
    elif choice == 4:
        mu, sigma, color = 0.5561, 0.0605, "tab:red"
    elif choice == 5:
        mu, sigma, color = 0.5985, 0.0654, "tab:purple"

    segment_vals = random_state.normal(mu, sigma, Ti)
    
    segment_lbls = (segment_vals > mu).astype(int)
...
# plot data stream and the data distribution histogram
def plot_stream_and_hist(stream, drifts, segment_lengths, segment_colors, random_state):
    ...
    dist_info = [
        (0.6258, 0.0470, "tab:blue", "A"),
        (0.3745, 0.0204, "tab:orange", "B"),
        (0.5760, 0.0288, "tab:green", "C"),
        (0.5561, 0.0605, "tab:red", "D"),
        (0.5985, 0.0654, "tab:purple", "E")
    ]
    
    for mu, sigma, color, label in dist_info:
        ideal_dist = random_state.normal(mu, sigma, 2000)
        ax2.hist(ideal_dist, bins=20, alpha=0.5, label=label, color=color)
